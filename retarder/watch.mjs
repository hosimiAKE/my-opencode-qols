...
const LOG = path.join(HERE, "retarder.log");
const INTERVAL_MS = Number(process.env.RETARDER_INTERVAL_MS || 20000);
...
function log(msg) {
  ...
  if (!process.env.RETARDER_QUIET) console.log(line);
}
...
function runPatch(args) {
  const r = spawnSync(process.execPath, [PATCH, ...args, "--quiet"], { encoding: "utf8", windowsHide: true });
  return r;
}
